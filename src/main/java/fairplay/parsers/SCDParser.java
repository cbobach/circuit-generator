...
public class SCDParser implements CircuitParser<List<GateConstructor>> {
	...
	public List<List<GateConstructor>> getGates() {
		...
		List<GateConstructor> gates = new ArrayList<GateConstructor>();
		...
		try {
			...
			for (int i = 0; i < numberOfGates; i++) {
				...
				GateConstructor g = new GateConstructor(gate, InputGateType.FAIRPLAY);
				gates.add(g);
			}
