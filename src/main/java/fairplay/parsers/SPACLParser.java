...
public class SPACLParser implements CircuitParser<List<GateConstructor>> {
		...
		try {
			...
			while((line = fbr.readLine()) != null) {
				...
				if (line.contains(CommonUtilities.MAX_WIDTH + CommonUtilities.XOR)) {
					...
					layersOfGates.get(i).add(getGate(line, GateType.XOR));
				} else if (line.startsWith(CommonUtilities.INV + "(")) {
					layersOfGates.get(i).add(getGate(line, GateType.INV));
					numberOfNonXORGates++;
				} else if (line.startsWith(CommonUtilities.AND + "(")) {
					layersOfGates.get(i).add(getGate(line, GateType.AND));
					numberOfNonXORGates++;
				}
			}
			fbr.close();
	...
	private GateConstructor getGate(String line, GateType type) {
		String[] split = line.split(",");
		if (type == GateType.INV) {
			String output = split[0].substring(4);
			String leftInput = split[1];
			String gateString = "1 1 " + leftInput + " " +
					output + " " + "-1";
			return new GateConstructor(gateString, InputGateType.FAIRPLAY);
		} else {
			String gateType = "";
			String output = split[0].substring(4);
			String leftInput = split[1];
			...
			if (type.equals(GateType.XOR)) {
				gateType = "0110";
			} else if (type.equals(GateType.AND)) {
				gateType = "0001";
			}
