...
public class FairplayParser implements CircuitParser<GateConstructor> {
    ...
    HashMap<Integer, GateConstructor> outputMap;
    ...
    public FairplayParser(File circuitFile, boolean stripWires) {
        ...
        this.outputMap = new HashMap<Integer, GateConstructor>();
    }
        ...
        try {
            ...
            while ((line = fbr.readLine()) != null) {
                ...
                if (line.endsWith("INV")) {
                    line = "1 " + split[1] + " " + split[2] + " " + split[3] + " -1";
                } else if (line.endsWith("XOR")) {
                    line = split[0] + " " + split[1] + " " + split[2] + " " + split[3] + " " + split[4] + " " + "0110";
                } else if (line.endsWith("AND")) {
                    line = split[0] + " " + split[1] + " " + split[2] + " " + split[3] + " " + split[4] + " " + "0001";
                }

				/*
				 * Construct each gate and count number of AND gates
				 */
                GateConstructor g = new GateConstructor(line, InputGateType.FAIRPLAY);
                addToWireInfo(g);
    ...
    private void addToWireInfo(GateConstructor g) {
    ...
    private void stripBlankWires(List<GateConstructor> res) {
        ...
        // is sorted by fairplay.output wires
        for (int i = originalNumberOfWires - 1; i >= 0; i--) {
            boolean b = blankWires[i];
            if (!b) {
                for (int j = i; j <= originalNumberOfWires; j++) {
                    GateConstructor outputG = outputMap.get(j);
                    if (outputG != null) {
                        outputG.setOutputWireIndex(outputG.getOutputWireIndex() - 1);
                    }

                    Collection<GateConstructor> leftWires = leftMap.getCollection(j);
                    if (leftWires != null) {
                        for (GateConstructor leftG : leftWires) {
                            leftG.setLeftWireIndex(leftG.getLeftWireIndex() - 1);
                        }
                    }

                    Collection<GateConstructor> rightWires = rightMap.getCollection(j);
                    if (rightWires != null) {
                        for (GateConstructor rightG : rightWires) {
